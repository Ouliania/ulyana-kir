...
description: "Apply psychological principles, mental models, and behavioral science to marketing decisions. Use when the user mentions 'psychology,' 'mental models,' 'cognitive bias,' 'persuasion,' 'behavioral science,' 'why people buy,' 'decision-making,' 'consumer behavior,' 'anchoring,' 'social proof,' 'scarcity,' 'loss aversion,' 'framing,' or 'nudge.' Reach for it when someone wants to understand or leverage how people think and decide in a marketing context — copy review, page audits, positioning, CTA design, pricing, growth loops."
---
...
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename), read it before applying mental models. Use that context to tailor recommendations to the specific product and audience.
...
Mental models are thinking tools — they sharpen strategy and help you solve the right problems before reaching for tactics.
...
The simplest explanation is usually correct. Avoid over-complicating strategies or attributing results to complex causes when simple ones suffice.
...
## How to Apply on This Project

When the user asks about copy, CTAs, case cards, or service pages on ulyanaweb, **hand off to the `marketing-strategist` custom agent** (`.commandcode/agents/marketing-strategist.md`). That agent owns the project's marketing voice, taste rules, and audit workflow.

This skill provides the psychology; the agent provides the voice. Use both: read this skill to know which model applies, then let the agent draft in the founder's actual tone.
